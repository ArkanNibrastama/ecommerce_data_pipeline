...
spark = SparkSession.builder.master("spark://10.227.33.118:7077").appName("tes").getOrCreate()
...
json_path = '../data/'
json_file_names = [n for n in os.listdir(json_path) if n.endswith('.json')]
for json_file_name in json_file_names:
    input_json = spark.read.format('json').load(json_path+json_file_name)
...
# df.printSchema()
# df.show()
df.coalesce(1).write.option('header', True).mode('overwrite').csv('../transformed_data/'+datetime.now().strftime('%Y%m%d')+'/')
# df.write.mode('overwrite').csv('../transformed_data/')
# df.write.csv('../transformed_data'+datetime.now().strftime("%Y-%m-%d %H:%M:%S")+".csv")
